...
fit_model_fn <- function(date_val = "2007-11-06",
...
process_time_periods <- function(TIME_PERIOD, knot_range = 1:4) {
  ...
  for (date in unique_dates) {
    for (knots in knot_range) {
      ...
      res <- fit_model_fn(date_val = date, num_knots = knots, 
...
df_results_spline <- process_time_periods(df_pr$TIME_PERIOD)

write.csv(df_results_spline, "../data/japan_df_results_spline.csv", row.names = F)

df_results_spline = read.csv(../data/japan_df_results_spline.csv)
...
plot_line_fn_errs <- function(col_sel){
  df_plot <- df_results_spline %>%
    select(all_of(col_sel), TIME_PERIOD, Knots) %>%
